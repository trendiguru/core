...
constants.paperdoll_ttl = float(300)
    ...
    for file in listing:
       #     if Utils.good_bb(img, skip_if_marked_to_skip=True) and good_img(img):
        ...
        while max_retry and not got_mask:
            try:
                paper_job = paperdoll_parse_enqueue.paperdoll_enqueue(img_arr, at_front=True, async=False, queue_name="pd_yuli")
                mask, labels, pose = paper_job.result[:3]
                ...
                for num in np.unique(final_mask):
                    ...
                    print category
                    # if len(category) > 0 and category in constants.paperdoll_shopstyle_women.keys():
                    #     print("Found dress!!")
