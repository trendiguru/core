 ...
 #                                                           {'object':'argentina7','bbox_xywh':[x,y,w,h]}
    ...
    def onmouse(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONUP:
            ...
            print('button up {} '.format(self.track_window))
            if self.object_being_edited is not None:
                self.selections[self.object_being_edited] = self.track_window
                print('object {} window {}'.format(self.object_being_edited,self.selections[self.object_being_edited]))
...
#
    ...
    def run(self):
        ...
        if movie:
            ...
            out = cv2.VideoWriter(output_moviename,fourcc, 20.0, (1280,720))
            h,w=720,1280
...
#            ret, self.frame = self.cam.read()
        ...
        # video
            ...
            #             x1=x0+w
            #             y1=y0+h
            #             print('selection {} is xywh {} , x1x2y1y2 {} {} {} {} rect'.format(i,selection,x0,y0,x1,y1))
            #             cv2.rectangle(frame, (x0,y0),(x1,y1), (0,255,255))
            #             hsv_roi = hsv[y0:y1, x0:x1]
            #             mask_roi = mask[y0:y1, x0:x1]
            #             hist = cv2.calcHist( [hsv_roi], [0], mask_roi, [16], [0, 180] )
            #             cv2.normalize(hist, hist, 0, 255, cv2.NORM_MINMAX)
            #             self.hists[i] = hist.reshape(-1)
            #             self.show_hists(i)
            #
            #             vis_rois[i] = vis[y0:y1, x0:x1]
            #             cv2.bitwise_not(vis_rois[i], vis_rois[i])
            #             vis[mask == 0] = 0
            #             i=i+1
            #
            #
    ...
    def adjust_box(self,object_ind=None):
        ...
        while(1):
            ...
            while(1):
                ...
                print('(k)ill, arrows or awsd to move, (r)edraw, (q)uit')
                inp = cv2.waitKey(0)
                bb = the_annotation['bbox_xywh']
    ...
    def draw_bbs(self,annotations,image=None):
        for object in annotations:
            ...
            if image is not None:
                self.frame = bb_with_text(image,bbox,str(id))
            else:
                self.frame = bb_with_text(self.frame,bbox,str(id))
        return self.frame
...
#text at bottom
    ...
    cv2.putText(img_arr,text,(bb_xywh[0]+5,bb_xywh[1]+bb_xywh[3]-margin+margin/2),cv2.FONT_HERSHEY_PLAIN, 1, text_color)
