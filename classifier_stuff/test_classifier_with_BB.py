        ...
        for j in i:
            ...
            if a == 0:  #this is prob unneeded as long as n_samples[i] != 0
                ...
                if den > 0:
                    ...
                    print('l:' + str(l))
            print(
                'detected:' + str(j) + ' tot targets' + str(n_samples[n]) + ' value:' + str(tmp_arr[-1]) + 'sum:' + str(
                    classifier_sum))
            n = n + 1
    ...
    for dirName, subdirList, fileList in os.walk(imageDirectory, topdown=True):
        for file in fileList:
            ...
            if img_array is not None:
                ...
                print('nTargets:' + str(nTargets) + ' tot:' + str(totTargets) + ' nMatches:' + str(
                    nMatches) + ' tot:' + str(totMatches) + ' nFalse:' + str(nFalseMatches) + ' tot:' + str(
                    totFalseMatches) + ' name:' + str(full_name))
...
def test_classifiers(trainDir='057', imageDir='images/cjdb'):
    ...
    for i in range(0, n_classifiers):
        ...
        for j in range(0, n_categories):
            if check_empty:
                print('in test_classifier.py:classify_image: classifier ' + classifierNames[i] + ' is empty!! ',
                      str(check_empty))
                totTargets, totMatches, totFalseMatches = [0, 0, 0]
            else:
                print('cascade:' + classifierNames[i] + ' directory:' + imageDirectories[j] + ' search string:' +
                      searchStrings[j])
                totTargets, totMatches, totFalseMatches = test_classifier(cascade, imageDirectories[j],
                                                                          searchStrings[j])

            print('totTargets:' + str(totTargets) + ' totMatches:' + str(totMatches) + ' tot FalseMatches:' + str(
                totFalseMatches) + '               ')
            results_dict = {'classifier': classifierNames[i], 'directory': imageDirectories[j],
                            'search string': searchStrings[j], 'totTargets': totTargets, 'totMatches': totMatches,
                            'FalseMatches': totFalseMatches, 'time': time.strftime('%c')}
            results_row.append(results_dict)
            results_list.append(results_row)
            if i == j:
                results_matrixrow.append(
                    totMatches - totFalseMatches)  #think about whether this is right for category i,j and for category i,i
            else:
                results_matrixrow.append(
                    totMatches + totFalseMatches)  #think about whether this is right for category i,j and for category i,i
...
def main(argv):
    ...
    parser.add_argument('-o', dest='outputDir',
                        default='/home/jeremy/jeremy.rutman@gmail.com/TrendiGuru/techdev/databaseImages/googImages/output/',
                        help='output dir')
    parser.add_argument('-i', dest='imageDir',
                        default='/home/jeremy/jeremy.rutman@gmail.com/TrendiGuru/techdev/databaseImages/googImages/shirt/longSleeve/BUTTONSHIRT/',
