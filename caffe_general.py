        ...
        if not async:
            ...
            if delete_when_done:
                try:
                    ...
                    logging.warning(
                        "caffe_general could not delete file {0} because of exception: \n{1}".format(filename, e))
            print(job.result)
            return job.result
        else:
...
def ask_caffe(filename):
    ...
    detect_command = "{bin_path} {prototxt} {caffemodel} {binproto} {words} {img}" \
        .format(bin_path=bin_path, prototxt=prototxt, caffemodel=caffemodel, binproto=binproto, words=words,
                img=filename)
...
if __name__ == "__main__":
    ...
    for url in urls:
        i += 1
        print('url #' + str(i) + ' ' + url)
        cats_dict = caffe_general(url, async=False)
        print(str(cats_dict))
