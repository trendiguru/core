...
class ShopStyleDownloader():
    ...
    def archive_products(self, category_id=None):
        ...
        if category_id:
            query_doc["$and"].append(
                {"categories":
                     {"$elemMatch": {"id": {"$in": dbUtils.get_all_subcategories(self.db.categories, category_id)}}}
                 })

        update_result = self.db.products.update_many(query_doc, {"$set": {"archive": True}})
        # for prod in old_prods:
        #     self.db.archive.insert(prod)
        #     self.db.products.delete_one({"id": prod["id"]})
        print "Marked {0} of {1} products for archival".format(update_result.modified_count, update_result.matched_count)
    ...
    def shopstyle_fields_update(self, prod):
        ...
        """
        self.db.products.update_one({'id': prod['id']}, {'$set': prod})
    ...
    def insert_and_fingerprint(self, prod, do_fingerprint=None):
        ...
        prod["_id"] = self.db.products.insert_one(prod).inserted_id
        if do_fingerprint:
            print "enqueuing for fingerprinting...,",
            q.enqueue(super_fp, image_url=None, db_doc=prod, save_to_db=True)
        ...
        if prod_in_products is None:
            ...
            if prod_in_archive is None:
                ...
                if prod_in_archive.get("fp_version") == constants.fingerprint_version:
                    ...
                    self.db.products.insert_one(prod_in_archive)
                else:
            ...
            if prod_in_products.get("lastModified", None) != prod.get("lastModified", None):  # assuming shopstyle update it correctly
