...
#    cv2.imshow('test',in_)
    ...
    neurodoll_per_class_net.blobs['data'].reshape(1, *in_.shape)
    neurodoll_per_class_net.blobs['data'].data[...] = in_
    # run net and take argmax for prediction
    neurodoll_per_class_net.forward()
#    out = net.blobs['score'].data[0].argmax(axis=0) #for a parse with per-pixel max
    out = neurodoll_per_class_net.blobs['siggy'].data[0][category_index] #for the nth class layer #siggy is after sigmoid
    min = np.min(out)
    max = np.max(out)
