...
#   if i == pantsindex or i == jeansindex:
    ...
    for i in range(len(thresholded_multilabel)):
        if thresholded_multilabel[i]:
            ...
            unique_to_new_mask = np.where(item_mask != 0 and final_mask == 0)   #if two masks include same pixel take first, don't add the pixel vals together
            unique_to_new_mask = np.times(unique_to_new_mask,neurodoll_index)
