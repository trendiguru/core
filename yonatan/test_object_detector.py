...
def theDetector(url_or_np_array):
    ...
    # for d in dets:
        if dets[i].left() < 0:
            ...
            left = dets[i].left()
        print "dets[i].left: {1}, dets[i].top: {2}, dets[i].right: {3}, dets[i].bottom: {4}\n".format(left, dets[i].top(), dets[i].right(), dets[i].bottom())
        cv2.rectangle(padded_image, (left, dets[i].top()), (dets[i].right(), dets[i].bottom()), (0, 0, 255), 3)
