...
net = caffe.Net('/data/yonatan/yonatan_files/trendi/yonatan/alexnet/Alexnet_deploy_no_group.prototxt', '/data/yonatan/yonatan_caffemodels/genderator_caffemodels/caffe_alexnet_train_faces_iter_100000.caffemodel', caffe.TEST)
...
for num_layer, layer in enumerate(all_params):  # loops over different layers
    ...
    for i in range(len(net.params[
                           layer])):  # loops over stuff in layer - there's only 2 things with same len() in each layer, the first are all the weights, the second maybe mean of every filter (..?)
        ...
        print "len(params[0]): {0}".format(len(params[0]))
    ...
    for filter_num in range(len(params[0])):
        ...
        norm_R = LA.norm(filter[0])
        # print "filter_R: {0}".format(filter_R)
        # print "norm_R: {0}".format(norm_R)
        norm_G = LA.norm(filter[1])
        norm_B = LA.norm(filter[2])
