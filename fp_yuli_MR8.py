...
def trim_mask(small_image, mask):
    ...
    min_y = np.min(ind[0]);
    max_y = np.max(ind[0]);
    ry = np.abs(max_y - min_y)
    min_x = np.min(ind[1]);
    max_x = np.max(ind[1]);
    rx = np.abs(max_x - min_x)
...
def yuli_fp(trimmed_mask, feature_size):
    ...
    # cv2.imwrite('normfeature.jpg',feature)
...
if __name__ == "__main__":
    ...
    paths = [['/home/omer/core/results_yuli/str/', '/home/omer/core/textures/Brick_Bare/'],
             ['/home/omer/core/results_yuli/small_sq/', '/home/omer/core/textures/Metal_Bare/'],
             ['/home/omer/core/results_yuli/fl_sq/', '/home/omer/core/textures/Stone_Sharp/'],
             ['/home/omer/core/results_yuli/dots/', '/home/omer/core/textures/Grave/'],
             ['/home/omer/core/results_yuli/circles/', '/home/omer/core/textures/Smoke/']]
    ...
    for filename in listing:
        ...
        gray_img = cv2.cvtColor(small_image, cv2.COLOR_BGR2GRAY)

        #
        # first_head = background_removal.image_is_relevant(small_image)[1][0]
        # print 'old head size: ', first_head
        # relevant_head = is_relevant_head(first_head)
        #
        #
        # if relevant_head:
        #     resized_img = resize_by_head(small_image, first_head)
        # else:
        #     print 'Head too small'
        # print 'new head size: ',background_removal.image_is_relevant(resized_img)[1][0]
        #
        # mask = get_mask(resized_img)
        # trimmed_img = trim_mask(resized_img, mask)
        ...
        for idx, val in enumerate(response):
            ms_response.append(mean_std_pooling(val, 6))

        responses.append(ms_response)

    with open(path_out + 'BB' + '_MR8.pickle', 'w') as f:
