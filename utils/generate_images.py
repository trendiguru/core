...
def generate_images(img_arr, max_angle = 5,n_angles=10, max_offset_x = 100,n_offsets_x=1,  max_offset_y = 100, n_offsets_y=1,max_noise=0.2,n_noises=10,do_mirror=True,output_dir='./')
    '''
    ...
    :param do_mirror: work on orig and x-axis-flipped copy
    ...
    if n_angles <2:
        offset_range_angles = [0]
    else:
        offset_range_angles = np.arange(-max_angle, max_angle , max_angle*2 / (n_angles-1))
    if n_offsets_x <2:
        offset_range_x = [0]
    else:
        offset_range_x = np.arange(-max_offset_x, max_offset_x, max_offset_x*2/(n_offsets_x-1))
    if n_offsets_y <2:
        offset_range_y = [0]
    else:
        offset_range_y = np.arange(-max_offset_y, max_offset_y, max_offset_y*2/(n_offsets_y-1))
    if n_noises <2:
        offset_range_noise = [0]
    else:
        offset_range_noise = np.arange(0, max_noise, max_noise/(n_noises-1))
    ...
    if do_mirror:
        fimg=img_arr.copy()
        mirror_image = cv2.flip(img_arr,0)
