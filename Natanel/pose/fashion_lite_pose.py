#hands can be horinztonal and below
import cv2
import numpy as np
import os
import sys
...
def create_test_and_train_set(validation_fraction=0.1):
    ...
    current_directory_name = os.getcwd()
    directory_path = current_directory_name + '/fashion_only_TG_dataset_lite'
...
def load_data(dataset_directory_name, joints_list_filename, portion):
    ...
    for file_name in only_files[p0: p1]:
        ...
        image = cv2.imread(dataset_directory_name + '/' + file_name, 0)
	...
	image = image.reshape((128, 128, 1)) # for grayscale
        images.append(image)
...
def train_net():
    ...
    # dataset:
    ## training:
    training_dataset_directory_name = 'fashion_only_TG_dataset_lite/TG_training_images'
    training_joints_list_filename = 'fashion_only_TG_dataset_lite/TG_training_joints.hdf5'
    ## validation:
    validation_dataset_directory_name = 'fashion_only_TG_dataset_lite/TG_validation_images'
    validation_joints_list_filename = 'fashion_only_TG_dataset_lite/TG_validation_joints.hdf5'

    model_description = 'fashion_pose_lite_model_weights'
    dataset_Nof_steps = 1
...
#    model.load_weights(model_description + '.hdf5')
    for epoch_No in range(epoches_number):
        for step in range(dataset_Nof_steps):
            ...
            model.fit(X_train, Y_train, batch_size=size_batch, nb_epoch=10000, verbose=1, callbacks=[checkpointer],
