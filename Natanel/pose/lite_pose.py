...
from keras.layers import merge, Input, Dense, Dropout, Activation, Flatten, Reshape, Convolution2D, Lambda, MaxPooling2D, ZeroPadding2D
...
def upright_human(joints_location_vector):
    Right_ankle = joints_location_vector[0, :2]
    Right_knee = joints_location_vector[1, :2]
    Right_hip = joints_location_vector[2, :2]
    Left_hip = joints_location_vector[3, :2]
    Left_knee = joints_location_vector[4, :2]
    Left_ankle = joints_location_vector[5, :2]
    Right_wrist = joints_location_vector[6, :2]
    Right_elbow = joints_location_vector[7, :2]
    Right_shoulder = joints_location_vector[8, :2]
    Left_shoulder = joints_location_vector[9, :2]
    Left_elbow = joints_location_vector[10, :2]
    Left_wrist = joints_location_vector[11, :2]
    Neck = joints_location_vector[12, :2]
    Head_top = joints_location_vector[13, :2]

    uprightness = False

    # if (Head_top[1] < Neck[1]) and \
    #     (Neck[1] < (Left_hip[1]+Right_hip[1])/2) and \
    #     (Right_shoulder[1] < Right_elbow[1]) and \
    #     (Left_shoulder[1] < Left_elbow[1]) and \
    #     (Right_hip[1] < Right_knee[1]) and \
    #     (Left_hip[1] < Left_knee[1]):
    #     # if Right_elbow[1] < Right_wrist[1] and Left_elbow[1] < Left_wrist[1]:
    #     # if Right_knee[1] < Right_ankle[1] and Left_knee[1] < Left_ankle[1]:
    #     uprightness = True

    if (Head_top[1] < Neck[1]):
        if (Neck[1] < (Left_hip[1]+Right_hip[1])/2):
            if (Right_shoulder[1] < Right_elbow[1]):
                if (Left_shoulder[1] < Left_elbow[1]):
...
# def kparse(image, knum):
#     image = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
#     (h, w, d) = image.shape
#     im = image.reshape((h * w, d))
#     # apply k-means using the specified number of clusters and
#     # then create the quantized image based on the predictions
#     # knum=2
#     clt = MiniBatchKMeans(n_clusters=knum)
...
def fit_to_net_input_size(image, joints_location_vector, max_shape=(128, 128)):
    '''
    :param image:
    :param output_size:
    :param joints_location_vector:
...
def dataset_creation(dataset_name_str, dataset_list, number_of_angles_to_rotate=2):
    ...
    delta_angle = 20/number_of_angles_to_rotate
    angles = range(-delta_angle, delta_angle+1, delta_angle)
    index = 0
    vecs_dataset = []
    percent_done0 = 0
...
def create_test_and_train_set(validation_fraction=0.1):
    ...
    current_directory_name = os.getcwd()
    directory_path = current_directory_name + '/TG_dataset_lite'
...
def load_data(dataset_directory_name, joints_list_filename, portion):
    ...
    for file_name in only_files[p0: p1]:
        ...
        # print image_No
        joints_location_vector = joints_data[:, :, image_No]#.astype('float32')
        # print joints_location_vector
        image = cv2.imread(dataset_directory_name + '/' + file_name, 1) # 0 for grayscale, 1 for BGR
#	image = cv2.equalizeHist(image).reshape((image.shape[0], image.shape[1], 1))
        # color segregation:
        # image = kparse(image, knum)
        # plot_image_skeleton_for_testing(image, joints_location_vector)
#        joints_location_vector[:, :2] = joints_location_vector[:, :2] /  image.shape[0]
	joints_location_vector = joints_location_vector[:, :2]
        joints_location_vector = joints_location_vector.reshape(joints_location_vector.size)

        # # TODO: decide wether to keep it in binary + sigmoid or go back to continuose valu...
        # binary_joints_vector = np.zeros((joints_location_vector.shape[0]*8))
        # p = 0
        # for i in joints_location_vector:
        #     binary_joints_vector[p:p+8] = np.unpackbits(i.astype('uint8'))
        #     p += 8
        # joints.append(binary_joints_vector)
...
def train_net():
    ...
    # dataset:
    ## training:
    training_dataset_directory_name = 'TG_dataset_lite/TG_training_images'
    training_joints_list_filename = 'TG_dataset_lite/TG_training_joints.hdf5'
    ## validation:
    validation_dataset_directory_name = 'TG_dataset_lite/TG_validation_images'
    validation_joints_list_filename = 'TG_dataset_lite/TG_validation_joints.hdf5'

    model_description = 'pose_lite_model_weights_conving'
...
#        conv = Convolution2D(main_Nkernels, 3, 3, activation=act, border_mode='same')(conv)
#    conv = MaxPooling2D((2, 2), strides=(2, 2))(conv)
#    conv = Convolution2D(4, 1, 1, activation='sigmoid', border_mode='same')(conv)
...
#    fc = Dense(Nmain_fc_neurons/8, activation=act)(fc)
#    fc = Dropout(0.5)(fc)
    fc = Dense(Nmain_fc_neurons, activation=act)(fc)
#    fc = Dropout(0.5)(fc)
